import pytest

from notification_registry import NotificationChannel
from notification_registry import NotificationPriority
from notification_registry import NotificationType
from notification_registry import deserialize_message
from platform_handler.client import send_platform
from tests.utils.mocks import patched_publisher


class TestSendPlatform:
    @pytest.fixture
    def publisher(self, mocker):
        return patched_publisher(mocker)

    @staticmethod
    def _published_message(publisher):
        raw = publisher.publish.call_args.kwargs["message"].encode()
        return deserialize_message(raw)

    def test_publishes_to_platform_queue(self, publisher, delivery_failed_payload):
        send_platform(delivery_failed_payload)

        publisher.publish.assert_called_once()
        assert publisher.publish.call_args.kwargs["queue"] == NotificationChannel.PLATFORM.queue_name

    def test_notification_type_matches_payload(self, publisher, delivery_failed_payload):
        send_platform(delivery_failed_payload)

        msg = self._published_message(publisher)
        assert msg.metadata.notification_type == NotificationType.DELIVERY_FAILED

    def test_priority_forwarded_to_metadata(self, publisher, delivery_failed_payload):
        send_platform(delivery_failed_payload, priority=NotificationPriority.HIGH)

        assert self._published_message(publisher).metadata.priority == NotificationPriority.HIGH

    def test_default_priority_is_normal(self, publisher, delivery_failed_payload):
        send_platform(delivery_failed_payload)

        assert self._published_message(publisher).metadata.priority == NotificationPriority.NORMAL

    def test_channel_is_always_platform(self, publisher, delivery_failed_payload):
        send_platform(delivery_failed_payload)

        assert self._published_message(publisher).metadata.channel == NotificationChannel.PLATFORM

    def test_declare_queue_true(self, publisher, delivery_failed_payload):
        send_platform(delivery_failed_payload)

        assert publisher.publish.call_args.kwargs["declare_queue"] is True

    def test_unknown_payload_raises_value_error(self, publisher):
        class UnknownPayload:
            pass

        with pytest.raises(ValueError, match="Unknown payload type"):
            send_platform(UnknownPayload())  # type: ignore[arg-type]

    def test_publisher_context_manager_exits(self, publisher, delivery_failed_payload):
        send_platform(delivery_failed_payload)

        publisher.__exit__.assert_called_once()
